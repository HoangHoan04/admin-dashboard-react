    ...
    Object.values(obj).forEach((item: any) => {
      ...
      if (item.key) {
        ...
        tree[nodeKey] = {
          key: item.key,
          label: item.label,
          translationKey: item.translationKey || item.label,
          path: item.path,
          icon: item.icon,
  ...
  Object.values(routes).forEach((item: any) => {
    ...
    if (item.key && isShow) {
      const menuItem: any = {
        ...
        label:
          translateFn && (item.translationKey || item.label)
            ? translateFn(item.translationKey || item.label)
            : item.label,
        icon: item.icon,
        isShow: isShow,
  ...
  const traverse = (obj: any) => {
    Object.values(obj).forEach((item: any) => {
      ...
      if (item.key) {
        list.push({
          key: item.key,
          label: item.translationKey || item.label,
          path: item.path,
          icon: item.icon,
          translationKey: item.translationKey || item.label,
          isShow: item.isShow ?? true,
...
export const getRouteByPath = (path: string): RouteDetail | undefined => {
  ...
  if (found) {
    return {
      key: found.key,
      label: found.translationKey || found.label,
      path: found.path,
      icon: found.icon,
      translationKey: found.translationKey || found.label,

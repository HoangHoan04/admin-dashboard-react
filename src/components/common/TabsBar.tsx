import { getRouteByPath } from "@/common/routes";
    ...
    <div className="flex items-center justify-between border-b border-border bg-muted/30 px-4 h-12 select-none relative">
      ...
      >
        {visibleTabs.map((tab, index) => {
          ...
          const title =
            route && t(route.label)
              ? t(route.label)
              : t(`routes.${tab.translationKey?.replace("routes.", "")}`) ||
                tab.id;
      ...
      <div className="flex items-center gap-1.5 pl-4 border-l border-border h-6 my-auto shrink-0 z-25">
              ...
              {overflowTabs.map((tab) => {
                ...
                const title =
                  route && t(route.label)
                    ? t(route.label)
                    : t(
                        `routes.${tab.translationKey?.replace("routes.", "")}`,
                      ) || tab.id;
                const isPinned = pinnedTabs.includes(tab.id);
        ...
        <Button
          ...
          title={isMaximized ? t("tabsBar.restore") : t("tabsBar.maximize")}
          className="text-muted-foreground hover:text-foreground"
        >
          {isMaximized ? (
